...
define(function(require){
  window.popBox = (function(){
    var html = '<div id="J_p_pop" class="p_pop">\
        ...
        tmpQuora = '<span class="text" style="<%=style%>"><i class="<%=type%>"></i><%=text%></span>',
        tmpBtn = '<a href="javascript:void(0);" style="<%=style%>" class="pp_btn" data-seq="<%=i%>" data-type="<%=type%>"><%=text%></a>',
    ...
    setQuora = function(opt){
      var html = tmpQuora;
      html = html.replace(/<%=type%>/, opt.type).replace(/<%=style%>/, opt.style).replace(/<%=text%>/, opt.text);
    ...
    setNav = function(opts){
      ...
      for(var i = 0, j = opts.length; i < j; i++){
        var opt = opts[i];
        html += tmpBtn.replace(/<%=i%>/, i).replace(/<%=type%>/, opt.type).replace(/<%=style%>/, opt.style).replace(/<%=text%>/, opt.text);
        _nav.innerHTML = html;
    ...
    showBox = function(params){
      ...
      typeof params == 'string' && (opts.quora.text = params);
